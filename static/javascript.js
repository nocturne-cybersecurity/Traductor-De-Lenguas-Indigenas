...
class SintesisVozIndigena {
    ...
    cargarVoces() {
        ...
        const cargarVocesReal = () => {
            this.vocesDisponibles = window.speechSynthesis.getVoices();
            if (this.vocesDisponibles.length > 0) {
                console.log('✅ Voces sintetizadas cargadas:', this.vocesDisponibles.length);
            }
        };
        
        window.speechSynthesis.onvoiceschanged = cargarVocesReal;
        cargarVocesReal(); // Intentar cargar inmediatamente
    }

    /**
     * Reglas de pronunciación MEJORADAS por idioma
     * Basadas en patrones fonéticos auténticos
     */
    crearReglasFoneticas(idioma) {
        const reglas = {
            'nahuatl': {
                // Consonantes nahuatl específicas
                'x([aeiou])': 'sh$1',          // x + vocal = sh
                'tl': 'tl',                     // tl = tl (mantener)
                'tz': 'ts',                     // tz = ts
                'qu([ae])': 'k$1',              // qu + a/e = ka/ke
                'qu([io])': 'kw$1',             // qu + i/o = kwi/kwo
                'hu([aeiou])': 'w$1',           // hu + vocal = w
                'c([^aeiouh])': 'k$1',          // c + consonante = k
                'z': 's',                       // z = s
                'ch': 'ch',                     // ch = mantener
                // Vocales largas (representadas con í)
                'á': 'ah',
                'é': 'eh',
                'í': 'ee',
                'ó': 'oh',
            ...
            'maya': {
                // Maya clásico y yucateco
                'x': 'sh',                      // x = sh (fricativa palatal)
                'ch': 'ch',                     // ch = mantener
                'tz': 'ts',                     // tz = ts
                'ts': 'ts',                     // ts = mantener
                'pp': 'p',                      // pp = p glotalizada
                'tt': 't',                      // tt = t glotalizada
                'kk': 'k',                      // kk = k glotalizada
                'x\\b': 'sh',                   // x final = sh
                'qu': 'k',                      // qu = k
                'á': 'ah',
                'é': 'eh',
    ...
    /**
     * Aplica reglas de pronunciación avanzadas
     */
    convertirFoneticamente(texto, idioma) {
        let resultado = texto.toLowerCase();
        const reglas = this.crearReglasFoneticas(idioma);
        
        // Aplicar reglas de forma ordenada (las más específicas primero)
        Object.entries(reglas).forEach(([patron, reemplazo]) => {
            try {
                const regex = new RegExp(patron, 'gi');
                resultado = resultado.replace(regex, reemplazo);
            } catch (e) {
                // Si hay error en la regex, aplicar reemplazo literal
                resultado = resultado.replace(new RegExp(patron.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), reemplazo);
            }
        });
    ...
    /**
     * Obtiene parámetros de voz por idioma
     * (velocidad, tono, volumen optimizados)
     */
    obtenerParametrosVoz(idioma) {
        const parametros = {
            'es': { rate: 1.0, pitch: 1.0, volume: 1.0 },
            'nahuatl': { rate: 0.75, pitch: 1.1, volume: 1.0 },
            'maya': { rate: 0.70, pitch: 1.15, volume: 1.0 },
            'zapoteco': { rate: 0.75, pitch: 1.05, volume: 1.0 },
            'mixteco': { rate: 0.72, pitch: 1.12, volume: 1.0 },
            'totonaco': { rate: 0.75, pitch: 1.08, volume: 1.0 },
            'huichol': { rate: 0.73, pitch: 1.10, volume: 1.0 }
        };
        
        return parametros[idioma] || parametros['es'];
    }

    /**
     * Selecciona la mejor voz disponible
     */
    seleccionarVoz(idioma) {
        // Preferencia de voces por orden
        const preferencias = {
            'es': ['es', 'es-ES', 'es-MX', 'Spanish'],
            'nahuatl': ['es', 'es-ES', 'es-MX'],
            'maya': ['es', 'es-ES'],
            'zapoteco': ['es', 'es-MX', 'es-ES'],
            'mixteco': ['es', 'es-MX', 'es-ES'],
            'totonaco': ['es', 'es-ES'],
            'huichol': ['es', 'es-MX', 'es-ES']
        };

        const prefsIdioma = preferencias[idioma] || preferencias['es'];
        
        for (let pref of prefsIdioma) {
            const voz = this.vocesDisponibles.find(v => 
        ...
        return this.vocesDisponibles[0] || null;
    }
    ...
    hablar(texto, idioma = 'es') {
        ...
        // Aplicar reglas fonéticas para idiomas indígenas
        if (idioma !== 'es') {
            const textoFonetico = this.convertirFoneticamente(texto, idioma);
            utterance.text = textoFonetico;
            // Mostrar en consola la transcripción fonética
            console.log(`📢 [${idioma.toUpperCase()}] Original: "${texto}"\n       Fonética: "${textoFonetico}"`);
        } else {
            utterance.text = texto;
        }

        // Establecer idioma
        utterance.lang = idioma === 'es' ? 'es-ES' : 'es-ES'; // Base en español para todos
        
        // Obtener parámetros optimizados para el idioma
        const params = this.obtenerParametrosVoz(idioma);
        utterance.rate = params.rate;
        utterance.pitch = params.pitch;
        utterance.volume = params.volume;

        // Seleccionar voz
        const voz = this.seleccionarVoz(idioma);
        if (voz) {
            utterance.voice = voz;
        }
        ...
        utterance.onstart = () => {
            console.log(`🔊 Iniciando reproducción: ${idioma.toUpperCase()}`);
        };
     ...
     */
    obtenerInfoIdioma(idioma) {
        const info = {
            'nahuatl': {
                nombre: 'Náhuatl',
                descripcion: 'Lengua de los mexicas, con sonidos como "x" (sh) y combinaciones como "tl"',
                caracteristicas: ['Sonido "x" = sh', 'Combinación "tl" importante', 'Vocales alargadas']
            },
            'maya': {
                nombre: 'Maya',
                descripcion: 'Lengua clásica con sonidos guturales y consonantes glotalizadas',
                caracteristicas: ['Sonido "x" = sh', 'Consonantes glotalizadas (pp, tt, kk)', 'Estructura monosilábica']
            },
            'zapoteco': {
                nombre: 'Zapoteco',
                descripcion: 'Lengua tonal con fricativas variadas',
                caracteristicas: ['Fricativas especiales', 'Tonos variables', 'Sonidos zh']
            },
            'mixteco': {
                nombre: 'Mixteco',
                descripcion: 'Lengua tonal con consonantes africadas complejas',
                caracteristicas: ['Africadas: tx, dx', 'Sonidos ñ', 'Estructura tonal']
            },
            'totonaco': {
                nombre: 'Totonaco',
                descripcion: 'Lengua con fricativas laterales y nasales nasalizadas',
                caracteristicas: ['Fricativa lateral lh', 'Nasales especiales', 'Glotalizaciones']
            },
            'huichol': {
                nombre: 'Huichol',
                descripcion: 'Lengua del pueblo Wixárika con estructura vocal compleja',
                caracteristicas: ['Sonido wi importante', 'Estructura silábica clara', 'Vocales alargadas']
            }
        };
    ...
    mostrarErrorNavegador() {
        alert('⚠️ Tu navegador no soporta síntesis de voz.\n\nPrueba con:\n✓ Google Chrome\n✓ Microsoft Edge\n✓ Safari\n✓ Firefox (versiones recientes)');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const languages = [
        { filename: 'JSON/mixteco.JSON', label: 'Mixteco' },
        { filename: 'JSON/nahuatl.JSON', label: 'Náhuatl' },
        { filename: 'JSON/totonaco.JSON', label: 'Totonaco' },
        { filename: 'JSON/zapoteco.JSON', label: 'Zapoteco' },
        { filename: 'JSON/maya.JSON', label: 'Maya' },
        { filename: 'JSON/huichol.JSON', label: 'Huichol' }
    ];
